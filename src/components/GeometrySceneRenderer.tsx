...
function renderLabel(
  ...
  switch (object.type) {
    ...
    case "segment": {
      ...
      return <TextLabel key={`${object.id}-label`} x={middle.x + 0.15} y={svgY(middle.y) - 0.16} text={label} shapeLabel />;
    }

    case "polygon": {
      const polygonPoints = object.points.map((id) => points.get(id)).filter(Boolean) as PointObject[];
      const center = centroid(polygonPoints);
      return <TextLabel key={`${object.id}-label`} x={center.x - 0.18} y={svgY(center.y + 0.58)} text={label} shapeLabel />;
    }
    ...
    case "circle": {
      ...
      if (!center) return null;
      const radius = getCircleRadius(object, points);
      return <TextLabel key={`${object.id}-label`} x={center.x + radius + 0.55} y={svgY(center.y + 0.1)} text={label} shapeLabel />;
    }
...
function TextLabel({ x, y, text, shapeLabel = false }: { x: number; y: number; text: string; shapeLabel?: boolean }) {
  return (
    <text x={x} y={y} className={`geometry-label ${shapeLabel ? "is-shape-label" : ""}`} vectorEffect="non-scaling-stroke">
      {text}
    </text>
  );
}
